...
Use `presentations/` for source decks and `reports/` for rendered or shareable outputs. The default structure includes `presentations/investor/` for a concise external pitch deck and `presentations/internal/` for a fuller internal business plan deck. Start from HTML-PPT templates and layouts, use token-based themes, and include speaker notes for human review.
